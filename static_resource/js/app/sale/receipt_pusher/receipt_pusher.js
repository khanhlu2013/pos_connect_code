...
{
    function ask_server_to_process_sale_data(store_db_name,callback){
        var data_ = {
    ...
    function sync_receipt(store_db_name,couch_server_url,callback){
        ...
        Pouch_db.replicate(store_db_name/*from local source*/, store_db_url/*to remote target*/,{filter:receipt_filter},function(err,resp){
            callback(err);                
        });
    }

    function push_receipt(store_idb,store_pdb,store_db_name,couch_server_url,callback){
        ...
        async.waterfall([receipt_lst_getter_b],function(error,result){
            ...
            var sync_receipt_b = sync_receipt.bind(sync_receipt,store_db_name,couch_server_url);
