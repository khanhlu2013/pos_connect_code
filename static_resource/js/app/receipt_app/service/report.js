    ...
    ,function(
        ...
        return function(){
            ...
            var template_date_panel = 
                '<input type="text" datepicker-popup="MMMM-dd-yyyy" ng-model="$parent.date_from" is-open="$parent.is_from_date_open" ng-click="open_from_date($event)" placeholder="from date">' +
                '<input type="text" datepicker-popup="MMMM-dd-yyyy" ng-model="$parent.date_to"   is-open="$parent.is_to_date_open"   ng-click="open_to_date($event)"   placeholder="to date">' +                    
            ...
            var template_right_table = 
                '<table ng-hide="$parent.cur_receipt==null" class="table table-hover table-bordered table-condensed table-striped">' +
                    ...
                    '<tr ng-repeat="receipt_ln in $parent.cur_receipt.receipt_ln_lst">' +
                        ...
                        '<td>{{receipt_ln.get_total_discount_price() | currency}}</td>' +
                        '<td>{{receipt_ln.get_otd_wt_price($parent.cur_receipt.tax_rate) | currency}}</td>' +                                    
                        '<td>{{receipt_ln.get_line_total($parent.cur_receipt.tax_rate) | currency}}</td>' +
                    '</tr>' +
                '</table>'
            ;
            ...
            var ModalCtrl = function($scope,$modalInstance,$http,$filter){
                ...
                $scope.tableParams = new ngTableParams({
                    ...
                    getData:function($defer,params){
                        ...
                        var promise = api.get_receipt($scope.date_from,$scope.date_to,params.page()/*page_no*/,params.count()/*receipt_per_page*/);
                        promise.then(
                            function(data){
