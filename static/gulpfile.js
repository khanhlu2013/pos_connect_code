...
gulp.task('_heroku_add', function(){
    return 
        gulp.src('./../**/*')
        .pipe(git.add({args: '-A'}));
});
gulp.task('_heroku_commit', function(){
    return 
        gulp.src('./../*')
        .pipe(git.commit('initial commit'));
});
gulp.task('_heroku_push', function(){
    return git.push('heroku', 'master', function (err) {
        if (err) throw err;
    });
});
gulp.task('heroku', ['_heroku_add','_heroku_commit','_heroku_push']);
gulp.task('s3_heroku', ['_build','_push_s3','heroku']);
