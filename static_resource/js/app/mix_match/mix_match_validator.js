    ...
    )
{
    ...
    function validate(name,qty,unit_discount,mix_match_child_lst){
        ...
        if(mix_match_child_lst.length == 0){
            error_lst.push(ERROR_MIX_MATCH_VALIDATION_CHILD_EMPTY);
        }else if(mix_match_child_lst.length > 1){
            var base = mix_match_child_lst[0];
            for(var i = 1;i<mix_match_child_lst.length;i++){
                if(
                       mix_match_child_lst[i].price != base.price
                    || mix_match_child_lst[i].crv != base.crv
                    || mix_match_child_lst[i].is_taxable != base.is_taxable
                )
                {
