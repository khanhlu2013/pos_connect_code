...
describe('no_sku_network_suggest -> select product dialog', function() {
    ...
    it('can calculate mod and median statistic for name,price,crv,cost,tax for product suggestion',function(){
        ...
        //---------------------------------------------------
        //verify statistic calculation confirm select product
        //---------------------------------------------------
        Select_product_dlg.click_col(0,'add');
        expect(Select_product_confirm_dlg.summary.name_lbl.getText()).toEqual('a');
        expect(Select_product_confirm_dlg.summary.price_lbl.getText()).toEqual('$5.00');
        expect(Select_product_confirm_dlg.summary.crv_lbl.getText()).toEqual('$1.00');        
        expect(Select_product_confirm_dlg.summary.cost_lbl.getText()).toEqual('$4.50');        
        expect(Select_product_confirm_dlg.summary.is_taxable.sign_span.getAttribute('class')).toEqual('glyphicon glyphicon-check');
        expect(Select_product_confirm_dlg.summary.is_taxable.percent_span.getText()).toEqual('(67%)');
    })
    ...
    it('can hide the percentage amount for 100 percent of is_taxable suggestion',function(){
        ...
        Select_product_dlg.click_col(0,'add');
        expect(Select_product_confirm_dlg.summary.is_taxable.sign_span.getAttribute('class')).toEqual('glyphicon glyphicon-check');
        expect(Select_product_confirm_dlg.summary.is_taxable.percent_span.isDisplayed()).toEqual(false);        
    })
    ...
    it('can hide the sign and amount for is_taxable suggestion when it is 50%',function(){
        ...
        //------------------------------------------------------------------------------------------
        Select_product_dlg.click_col(0,'add');
        expect(Select_product_confirm_dlg.summary.is_taxable.sign_span.isDisplayed()).toEqual(false);  
        expect(Select_product_confirm_dlg.summary.is_taxable.percent_span.isDisplayed()).toEqual(false);        
    })        
});