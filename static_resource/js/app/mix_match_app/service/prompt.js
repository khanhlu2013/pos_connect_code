	...
	mod.factory('mix_match_app/service/prompt',['$modal','service.ui.alert','sp_app.service.search_dlg.multiple',function($modal,alert_service,search_multiple_sp_service){
		return function(original_mm){
			var template = 
				...
				'<div class="modal-body">' +			
					'<form class="form-horizontal" name="form" novalidate>' +
 					...
 					'<table ng-hide="mm.mix_match_child_set.length==0" class="table table-hover table-bordered table-condensed table-striped">' +
 						'<tr>' +	
 							'<th>product</th>' +
 							'<th>remove</th>' +
 						'<tr>' +
 						'<tr ng-repeat="mm_child in mm.mix_match_child_set">' +
 						...
 						'</tr>' +
 					'</table>' +
 					'<label ng-show="mm.mix_match_child_set.length==0" class="error">empty deal</label>' +
 				'</div>' +

				'<div class="modal-footer">' +
					'<button ng-click="cancel()" class="btn btn-warning">cancel</button>' +
					'<button ng-disabled="is_unchange()" ng-click="reset()" class="btn btn-primary">reset</button>' +
					'<button ng-disabled="is_unchange() || (form.$invalid || mm.mix_match_child_set.length == 0)" ng-click="ok()"class="btn btn-success">ok</button>' +
				'</div>'						
			;
			var ModalCtrl = function($scope,$modalInstance,$filter,original_mm){
				...
				initial_mm = {is_include_crv_tax:false,mix_match_child_set:[]};

				if($scope.mm == null){
					$scope.mm = angular.copy(initial_mm);
				}
				$scope.remove_child = function(child){
					for(var i = 0;i<$scope.mm.mix_match_child_set.length;i++){
						if(child.id==$scope.mm.mix_match_child_set[i].id){
							$scope.mm.mix_match_child_set.splice(i,1);
							break;
						}
					}
				}
